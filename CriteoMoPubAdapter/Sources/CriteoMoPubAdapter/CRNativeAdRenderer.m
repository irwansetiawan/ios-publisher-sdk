...
@property(nonatomic, strong) CRNativeCustomEvent *adapter;
...
- (UIView *)retrieveViewWithAdapter:(CRNativeCustomEvent<MPNativeAdAdapter> *)adapter
                              error:(NSError **)error {
  if (!adapter || ![adapter isKindOfClass:[CRNativeCustomEvent class]]) {
    if (error) {
